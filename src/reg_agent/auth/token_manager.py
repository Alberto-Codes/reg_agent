...
class ImpersonatedTokenManager:
    ...
    def __init__(
        ...
        self._source_credentials, self._project_id = google.auth.default(scopes=scopes or ["https://www.googleapis.com/auth/cloud-platform"])
        ...
        if "@" in target_service_account_name_or_email:
            ...
            if not self._project_id:
                log.error("Could not determine project ID from source credentials to construct target SA email.")
                raise ValueError("Cannot construct target SA email: Project ID unknown and only SA name provided.")
        ...
        log.info(
            ...
            caller_sa=getattr(self._source_credentials, 'service_account_email', 'user_credentials'),
    ...
    def _is_token_valid(self) -> bool:
        ...
        return (self._token_expiry - REFRESH_BUFFER) > datetime.datetime.now(datetime.timezone.utc)

    def _generate_new_token(self) -> Tuple[str, datetime.datetime]:
        """Generates a new impersonated access token."""
        log.info("Generating new impersonated access token", target_sa=self.target_service_account)
        try:
            ...
            if not impersonated_creds.token or not impersonated_creds.expiry:
                log.error("Failed to obtain token/expiry from impersonated credentials.")
                raise RuntimeError("Could not generate impersonated token.")

            log.info("Successfully generated new impersonated token", expiry=impersonated_creds.expiry)
            ...
            raise RuntimeError(f"Failed to generate token for {self.target_service_account}") from e
...
async def _test_token_manager():
    ...
    import logging
    import os
    structlog.configure(processors=[structlog.stdlib.ProcessorFormatter.wrap_for_formatter], logger_factory=structlog.stdlib.LoggerFactory())
    ...
    try:
        ...
        manager = ImpersonatedTokenManager(target_service_account_name_or_email=target_sa_input, lifetime_seconds=60)
        token1 = manager.get_token()
        print(f"Got token 1 targeting {manager.target_service_account}: {token1[:10]}...{token1[-10:]}")
        print("Waiting for 30 seconds...")
        await asyncio.sleep(30)
        token2 = manager.get_token()
        print(f"Got token 2 targeting {manager.target_service_account}: {token2[:10]}...{token2[-10:]}")
        ...
        print(f"Got token 3 targeting {manager.target_service_account}: {token3[:10]}...{token3[-10:]}")
