...
async def run_task_3(engine: Engine):
    ...
    try:
        with get_session(engine=engine) as session:
            ...
            records_data = [
                (r.id, r.source_path, r.extracted_text)
    ...
    if not records_data:
        log.info("Task 3 Summary", found=records_found, success=success_count, errors=error_count)
    ...
    for record_id, source_path, extracted_text in records_data:
            ...
            try:
                with get_session(engine=engine) as update_session:
                    ...
                    if record:
                        ...
                        log.info("Updating status to FAILED_UNKNOWN", record_id=record_id)
                    else:
                         log.error("Could not find record to update status to FAILED_UNKNOWN", record_id=record_id)
            except Exception as commit_err:
                 log.error("Failed session/commit for FAILED_UNKNOWN status", record_id=record_id, error=str(commit_err))
            error_count += 1
        ...
        try:
            ...
            log.info("MetadataExtractionService initialized for record", record_id=record_id)
            ...
            try:
                with get_session(engine=engine) as update_session:
                    ...
                    if record:
                        ...
                        log.error("Could not find record to update metadata status", record_id=record_id, target_status=update_status)
                        # Adjust counts if record vanished?
                        if update_status == FileStatus.COMPLETED: success_count -=1
                        else: error_count -=1 # Decrement if we counted it
                        error_count += 1 # Add error for missing record

            except Exception as commit_err:
                log.error("Failed session/commit updating metadata status", record_id=record_id, error=str(commit_err))
            ...
            try:
                with get_session(engine=engine) as update_session:
                    record = update_session.get(FileRecord, record_id)
                    if record:
                        log.debug("Committing FAILED_METADATA status after API error", record_id=record_id)
                        record.status = FileStatus.FAILED_METADATA
                        update_session.add(record)
                        # Commit happens on session exit
                    else:
                        log.error("Could not find record to update status after API error", record_id=record_id)
            except Exception as commit_err:
                log.error("Failed session/commit updating FAILED_METADATA status after API error", record_id=record_id, error=str(commit_err))
            error_count += 1
            ...
            if metadata_service:
                try:
                    ...
                    log.info("MetadataExtractionService closed for record", record_id=record_id)
                except Exception as close_err:
                    log.error("Failed to close service for record", record_id=record_id, error=str(close_err))
            records_processed_ids.add(record_id)
