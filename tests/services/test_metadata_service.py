...
def mock_agent_constructor(mocker):
    ...
    dummy_metadata = RegulationDocumentMetadata(
        document_type="Mocked",
        issuing_agency="Mock",
        subject_institution="Mock",
        document_identifier="Mock-001",
    ...
    mock_constructor = mocker.patch(
        "reg_agent.services.metadata_service.Agent", return_value=mock_agent_instance
    )
...
def mock_openai_model_constructor(mocker):
    ...
    mock_constructor = mocker.patch(
        "reg_agent.services.metadata_service.OpenAIModel",
...
def mock_openai_provider_constructor(mocker):
    ...
    mock_constructor = mocker.patch(
        "reg_agent.services.metadata_service.OpenAIProvider",
...
def mock_google_auth(mocker):
    ...
    mock_default = mocker.patch(
        "reg_agent.services.metadata_service.google.auth.default",
        return_value=(mock_creds, "mock-project"),
    )
    mock_request = mocker.patch(
        "reg_agent.services.metadata_service.google.auth.transport.requests.Request"
    )
...
def mock_token_manager(mocker):
    ...
    mock_constructor = mocker.patch(
        "reg_agent.services.metadata_service.ImpersonatedTokenManager",
...
def mock_dynamic_auth(mocker):
    ...
    mock_constructor = mocker.patch(
        "reg_agent.services.metadata_service.DynamicBearerAuth",
...
def mock_httpx_client(mocker):
    ...
    mock_constructor = mocker.patch(
        "reg_agent.services.metadata_service.httpx.AsyncClient",
...
def test_init_direct_adc_success(
    mocker,
    mock_config,
    mock_google_auth,
    mock_openai_provider_constructor,
    mock_openai_model_constructor,
    ...
    mock_provider_constructor.assert_called_once_with(
        base_url=MOCK_BASE_URL, api_key="mock-adc-token"
    )
    ...
    mock_agent_constr.assert_called_once_with(
        model=mock_model_instance, output_type=RegulationDocumentMetadata
    )


def test_init_impersonation_success(
    mocker,
    mock_config,
    mock_token_manager,
    mock_dynamic_auth,
    ...
    mocker.patch(
        "reg_agent.services.metadata_service.TARGET_SA_NAME_OR_EMAIL", MOCK_TARGET_SA
    )
    ...
    mock_tm_constructor.assert_called_once_with(
        target_service_account_name_or_email=MOCK_TARGET_SA
    )
    ...
    # Check provider called correctly
    mock_provider_constructor.assert_called_once_with(
        base_url=MOCK_BASE_URL, http_client=mock_client_instance
    )
    # Check model called correctly
    mock_model_constructor.assert_called_once_with(
        MOCK_MODEL_NAME,
...
def test_init_direct_adc_fails_no_creds(mocker, mock_config):
    ...
    mock_openai_provider = mocker.patch(
        "reg_agent.services.metadata_service.OpenAIProvider"
...
def test_init_impersonation_fails_tm_init(mocker, mock_config):
    ...
    mocker.patch(
        "reg_agent.services.metadata_service.ImpersonatedTokenManager",
        side_effect=Exception("TM init failed"),
    )
    mock_openai_provider = mocker.patch(
        "reg_agent.services.metadata_service.OpenAIProvider"
    )  # Prevent downstream errors
...
@pytest.mark.asyncio
async def test_extract_metadata_agent_exception(
    mocker,
    mock_config,
    mock_google_auth,
    mock_openai_provider_constructor,
...
@pytest.mark.asyncio
async def test_close_with_impersonation(
    mocker,
    mock_config,
    mock_token_manager,
    mock_dynamic_auth,
    ...
    service = MetadataExtractionService()
    assert (
        service.http_client is mock_client_instance
    )  # Verify client was set during init
...
@pytest.mark.asyncio
async def test_close_with_direct_adc(
    mocker,
    mock_config,
    mock_google_auth,
    mock_openai_provider_constructor,
