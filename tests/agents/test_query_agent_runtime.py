...
from pydantic_ai import capture_run_messages  # Import capture_run_messages and models
from pydantic_ai.messages import (
    ...
    ToolReturnPart,
    UserPromptPart,
)  # Import message types and UserPromptPart, ModelMessage
...
)  # Added import for DuckDBToolDeps
...
FINAL_OUTPUT_TEXT_EXPLORE = (
    "Okay, here are the queryable metadata fields: issuing_agency, topic, year."
)


def mock_llm_explore_flow(
    ...
    if len(messages) == 1 and isinstance(last_request.parts[0], UserPromptPart):
        return ModelResponse(
            parts=[ToolCallPart(tool_name="explore_metadata", args={})]
        )
    ...
    # Second turn: Tool return for explore_metadata -> LLM gives final text
        ...
        if last_request.parts[0].tool_name == "explore_metadata":
            ...
            raise ValueError(
                f"Unexpected tool return in FunctionModel: {last_request.parts[0].tool_name}"
            )
...
async def test_agent_explore_metadata_flow():
    ...
    with capture_run_messages() as messages:
        ...
        result = await agent_instance.run(
            "What metadata fields can I query?", deps=tool_dependency
        )
