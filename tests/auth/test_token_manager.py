...
from reg_agent.auth.token_manager import ImpersonatedTokenManager, REFRESH_BUFFER
...
def mock_impersonated_creds_constructor(mocker):
    ...
    mock_instance.expiry = datetime.datetime.now(datetime.timezone.utc).replace(microsecond=0) + datetime.timedelta(hours=1)
...
def test_init_failure_google_auth_default_fails(mocker):
    ...
    with pytest.raises(google.auth.exceptions.DefaultCredentialsError):
        ImpersonatedTokenManager(target_service_account_name_or_email=MOCK_TARGET_SA_EMAIL)

def test_init_failure_construct_email_no_project_id(mock_google_auth_default_no_project):
    ...
    with pytest.raises(ValueError, match="Project ID unknown"):
        ImpersonatedTokenManager(target_service_account_name_or_email=MOCK_TARGET_SA_NAME)
...
def test_is_token_valid_no_token(mock_google_auth_default, mock_datetime_now):
    ...
    manager = ImpersonatedTokenManager(target_service_account_name_or_email=MOCK_TARGET_SA_EMAIL)
...
def test_is_token_valid_within_buffer(mock_google_auth_default, mock_datetime_now):
    ...
    manager._token_expiry = frozen_time + REFRESH_BUFFER - datetime.timedelta(seconds=10)
...
def test_is_token_valid_expired(mock_google_auth_default, mock_datetime_now):
    ...
    manager = ImpersonatedTokenManager(target_service_account_name_or_email=MOCK_TARGET_SA_EMAIL)
    manager._cached_token = MOCK_IMPERSONATED_TOKEN
    manager._token_expiry = frozen_time - datetime.timedelta(seconds=10) # Expired 10s before frozen time
    # mock_dt.now.return_value = frozen_time # Already set by fixture
    assert not manager._is_token_valid()
...
def test_is_token_valid_success(mock_google_auth_default, mock_datetime_now):
    ...
    manager._token_expiry = frozen_time + REFRESH_BUFFER + datetime.timedelta(minutes=10)
    # mock_dt.now.return_value = frozen_time # Already set by fixture
    assert manager._is_token_valid()

# --- Test Cases: _generate_new_token ---

def test_generate_new_token_success(mock_google_auth_default, mock_impersonated_creds_constructor):
    ...
    mock_creds_instance.refresh.assert_called_once_with(ANY) # ANY checks for a Request object
    assert token == MOCK_IMPERSONATED_TOKEN
    assert expiry == mock_creds_instance.expiry

def test_generate_new_token_constructor_fails(mock_google_auth_default, mock_impersonated_creds_constructor):
...
def test_generate_new_token_refresh_fails(mock_google_auth_default, mock_impersonated_creds_constructor):
    """Test failure when credentials refresh raises an exception."""
    mock_creds_constructor, mock_creds_instance = mock_impersonated_creds_constructor
    mock_creds_instance.refresh.side_effect = google.auth.exceptions.RefreshError("Refresh failed")
...
def test_generate_new_token_refresh_no_token(mock_google_auth_default, mock_impersonated_creds_constructor):
    ...
    mock_creds_instance.expiry = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=1)
...
def test_get_token_cached_valid(mock_google_auth_default, mock_datetime_now):
    ...
    manager._token_expiry = frozen_time + REFRESH_BUFFER + datetime.timedelta(minutes=30)
    # mock_dt.now.return_value = frozen_time # Already set by fixture
...
def test_get_token_no_cache(mock_google_auth_default, mock_impersonated_creds_constructor, mock_datetime_now):
...
def test_get_token_cache_expired(mock_google_auth_default, mock_impersonated_creds_constructor, mock_datetime_now):
...
def test_get_token_generation_fails(mock_google_auth_default, mock_impersonated_creds_constructor, mock_datetime_now):
    """Test get_token raises RuntimeError when token generation fails."""
    mock_creds_const, mock_creds_inst = mock_impersonated_creds_constructor
    mock_creds_inst.refresh.side_effect = RuntimeError("Original token gen failed") # Original error
    manager = ImpersonatedTokenManager(target_service_account_name_or_email=MOCK_TARGET_SA_EMAIL)
