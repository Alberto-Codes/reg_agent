...
from unittest.mock import (
    MagicMock,
)  # Can keep MagicMock if needed for complex mocks, but try to avoid.
...
def test_download_file_success(mocker, downloader_instance):
    ...
    mock_requests_get.assert_called_once_with(
        test_url, stream=True, headers=downloader.headers
    )
...
def test_download_file_request_exception(mocker, downloader_instance):
    ...
    mock_requests_get = mocker.patch(
        "requests.get", side_effect=requests.exceptions.RequestException("Test Error")
    )
...
def test_search_cfpb_success(mocker, downloader_instance):
    ...
    downloader = downloader_instance
    mock_fetch_parse = mocker.patch.object(
        downloader, "_fetch_and_parse", side_effect=mock_fetch
    )
    mock_find_download = mocker.patch.object(downloader, "_find_and_download_pdfs")

    # Expected detail URL to be passed to helpers
    expected_detail_url = (
        "https://www.consumerfinance.gov/actions/details/wells-fargo-action-1/"
    )
    ...
    mock_find_download.assert_called_once_with(
        mock_detail_soup, expected_detail_url, "CFPB"
    )
...
def test_search_occ_success(mocker, downloader_instance):
    ...
    mock_find_download.assert_called_once_with(
        mock_detail_soup, expected_detail_url, "OCC"
    )
...
MOCK_FRB_CSV_DATA = (
    "Institution Name,Docket Number,Action Type,Action Date,URL\n"
...
def test_search_frb_success(mocker, downloader_instance):
    ...
    # Assert
    mock_requests_get.assert_called_once_with(
        expected_csv_url, headers=downloader.headers
    )
...
def test_search_frb_csv_fetch_error(mocker, downloader_instance, caplog):
    ...
    mock_requests_get = mocker.patch(
        "requests.get",
        side_effect=requests.exceptions.RequestException("CSV Fetch Failed"),
    )
    ...
    # Assert
    mock_requests_get.assert_called_once_with(
        expected_csv_url, headers=downloader_instance.headers
    )
...
def test_find_and_download_pdfs_success(mocker, downloader_instance):
    ...
    mock_soup = BeautifulSoup(
        MOCK_CFPB_ACTION_DETAIL_HTML, "html.parser"
    )  # Reuse some html
    ...
    expected_calls = [
        mocker.call(
            "https://www.consumerfinance.gov/documents/consent-order-1.pdf",
            "TEST_consent-order-1.pdf",
        ),
        mocker.call(
            "https://www.consumerfinance.gov/actions/details/documents/consent-order-2.pdf",
            "TEST_consent-order-2.pdf",
        ),
