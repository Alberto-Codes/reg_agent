...
from reg_agent.core.db.connection import connect_db, DB_DIR
...
def test_connect_db_creates_table_and_file(tmp_path):
    ...
    try:
        ...
        assert isinstance(con, duckdb.DuckDBPyConnection), "Should return a DuckDB connection"
        ...
        assert len(columns_info) == len(EXPECTED_COLUMNS), f"Expected {len(EXPECTED_COLUMNS)} columns, found {len(columns_info)}"
        ...
        assert set(actual_columns.keys()) == set(EXPECTED_COLUMNS.keys()), "Column names do not match expected"
